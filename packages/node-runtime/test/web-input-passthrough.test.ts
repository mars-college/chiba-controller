...
test("node runtime accepts typed input passthrough when chromium backend is active", async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "c3-node-web-input-"));
...
`,
    ...
    const webUrl = `http://127.0.0.1:${webPort}/web-target`;
    webServer = http.createServer((req, res) => {
      const pathname = new URL(req.url ?? "/", "http://localhost").pathname;
      if (pathname === "/web-target") {
        res.statusCode = 200;
    ...
    const inputRes = await fetch(`http://127.0.0.1:${nodePort}/api/input`, {
      method: "POST",
    ...
    const inputJson = (await inputRes.json()) as {
      ...
      backend: string;
      command: string[];
      action: { kind: string; key?: string };
      code: number;
    };
    assert.equal(inputJson.ok, true);
    assert.equal(inputJson.backend, "chromium");
    assert.equal(inputJson.action.kind, "key");
    assert.equal(inputJson.action.key, "space");
    assert.equal(inputJson.code, 0);
